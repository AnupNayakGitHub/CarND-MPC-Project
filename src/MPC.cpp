...
constexpr double dt(){return 0.1;} //Timestep
constexpr size_t N(){ return 10;} //Timestep length

// Set cost factors
// TODO: tune cost factors
constexpr int cte_weight_factor() {return  2000;}
constexpr int epsi_weight_factor() {return 2000;} // made initial portion etc much less snaky
constexpr int v_weight_factor() {return 1;}
constexpr int current_delta_weight_factor() {return 10;}
constexpr int diff_delta_weight_factor(){ return 100;}
constexpr int current_a_weight_factor(){ return 10;}
constexpr int diff_a_weight_factor(){ return 10;}
...
constexpr double ref_cte() {return  0;}
constexpr double ref_epsi() {return 0;}
constexpr double ref_v() {return 80;}
...
constexpr size_t x_start() { return 0;}
constexpr size_t y_start() { return x_start() + N();}
constexpr size_t psi_start() {return y_start() + N();}
constexpr size_t v_start() { return psi_start() + N();}
constexpr size_t cte_start() { return v_start() + N();}
constexpr size_t epsi_start() { return cte_start() + N();}
constexpr size_t delta_start() { return epsi_start() + N();}
constexpr size_t a_start() { return delta_start() + N() - 1;}
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N(); i++) {
      fg[0] += cte_weight_factor()*pow(vars[cte_start() + i] - ref_cte(), 2);
      fg[0] += epsi_weight_factor()*pow(vars[epsi_start() + i] - ref_epsi(), 2);
      fg[0] += v_weight_factor()*pow(vars[v_start() + i] - ref_v(), 2);
      
    }
    
    // (2) Cost increases with use of actuators
    for (int i = 0; i < N() - 1; i++) {
      fg[0] += current_delta_weight_factor()*pow(vars[delta_start() + i], 2);
      fg[0] += current_a_weight_factor()*pow(vars[a_start() + i], 2);
    }
    
    // (3) Cost increases with value gap between sequential actuators
    for (int i=0; i < N()-2; i++) {
      fg[0] += diff_delta_weight_factor()*pow(vars[delta_start() + i + 1] - vars[delta_start() + i], 2);
      fg[0] += diff_a_weight_factor()*pow(vars[a_start() + i + 1] - vars[a_start() + i], 2);
    }
